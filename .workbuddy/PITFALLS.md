...
## #18 2026-07-22: 大运 5 维空内容 — max_tokens 800 截断 6 段
- **现象**: 35-44 大运综合解读 300 字（成功），5 维全部 0 字；但 55-64 大运 5 维 5/5 成功
- **根因**: max_tokens=800 装不下 6 段 × 60-80 字 + 5 个 `|||` ≈ 600+ 字；当 LLM 综合段 100 字 + 5 维 80 字 = 500+ 字时，剩余 token 不够 5 维全部输出
- **修复**（v8.0.2）: dayun max_tokens 800→1200；同时把各维度字数要求从 60-80 改为 50-65（更稳）
- **教训**: 多段结构化输出按"段数 × 每段上限 + 分隔符数"计算 max_tokens；不要复用 summary 的 800 经验

## #19 2026-07-22: 流月 12 月 3 列 grid 被截断 → 末尾 2 月空白
- **现象**: 截图显示 2028/2029 流年末尾"月"块是空的（蓝圈标注）
- **根因**: 12 个月用 3 列 grid（4 行），在表格视口下右侧 2-3 块被裁切/挤到视口外，看起来"空白"
- **修复**（v8.0.2）: 改 12 月 fallback 为横向滚动（display:flex + overflow-x:auto），与 LLM 模式一致
- **教训**: 多卡片在表格列内显示时，优先用横向滚动而非 grid（避免被表格列宽裁切）

## #17 2026-07-22: 玄学称谓/时事强制引用 → 用户反馈刻意
- **现象**: LLM 输出的"老衲/师/鄙人"等自称被用户认为矫揉造作
- **用户反馈**: "用玄学专家的口吻过于刻意，结合时事去点评不需要这么刻意去提及，应该结合当今社会情况以及命盘内容对应去分析和表述"
- **根因**: Prompt 中显式要求"必须以老衲/师/鄙人开头"+"必须引用至少2个2024-2026时事"，导致 LLM 输出生硬、模板化
- **修复**（v8.0.2）: 改 Prompt 为"结合当下时代背景（经济结构、婚育观念、养老医疗、就业市场等）与命盘格局对应分析，无需生硬列举时事" + "专业、务实、有温度，避免矫揉造作"
- **教训**: Prompt 的语气约束要克制，过度具体会反向劣化输出；让 LLM 自然融合更优
