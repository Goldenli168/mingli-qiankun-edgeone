...
直接输出3段内容。"""
        ...
        sc = ctx.get('scores','')
        prompt = f"""你是资深命理分析师。请为以下大运给出综合解读与5维分析。
...
严格用【字段名】前缀分6段:
【综合】180-200字（整体基调、十年核心走向、关键转折节点、需重点布局与避坑的领域）
【财富】80-100字（结合经济周期与行业趋势，给可操作的理财、资产配置、副业建议）
【事业】80-100字（结合职场生态、年龄阶段与升迁规律，给发展策略、人脉经营、避坑建议）
【婚姻】80-100字（结合当代婚恋观与家庭阶段，给感情经营、伴侣沟通、姻缘时机建议）
【子女】80-100字（结合教育趋势与代际特征，给教养规划、亲子沟通、子女发展方向）
【父母】80-100字（结合养老医疗现状与代际责任，给孝亲陪伴、长辈健康、经济支持建议）

结合时代背景自然融入，给可操作的方向性指引，语气专业、务实、有温度。直接输出。"""

    elif gen_type == "dayun_brief":
        # 精简版:【综合180+5维80-90】6段,用于未来大运
        sc = ctx.get('scores','')
        prompt = f"""你是资深命理分析师。请为以下大运给出综合+5维分析:
...
严格按【字段名】前缀输出6段:
【综合】180字(整体基调、十年核心走向、关键转折节点、需重点布局)
【财富】80-90字(结合经济周期与行业趋势,给理财与资产配置建议)
【事业】80-90字(结合职场生态,给发展策略与避坑建议)
【婚姻】80-90字(结合当代婚恋观,给感情经营建议)
【子女】80-90字(结合教育趋势,给教养规划建议)
【父母】80-90字(结合养老医疗,给孝亲陪伴建议)

结合时代背景自然融入,给可操作指引,语气专业务实。直接输出。"""
