...
严格用【字段名】前缀分6段:
【综合】约200字（整体基调、十年走向、核心课题与转折点、需重点布局的领域）
【财富】约70字（结合行业与经济周期，给可操作的理财与资产配置方向）
【事业】约70字（结合职场生态与升迁规律，给发展策略与避坑建议）
【婚姻】约70字（结合当代婚恋观念，给感情经营与家庭和谐建议）
【子女】约70字（结合教育趋势与代际沟通，给教养规划方向）
【父母】约70字（结合养老医疗现实，给孝亲与陪伴建议）

结合时代背景自然融入分析，给可操作的方向性指引，语气专业、务实、有温度。直接输出。"""

    elif gen_type == "dayun_brief":
        # 精简版:输出【综合+5维精简】6段,每段50-60字,用于未来大运
        sc = ctx.get('scores','')
        prompt = f"""你是命理分析师。请为以下大运给出综合+5维精简分析(各50-60字):
