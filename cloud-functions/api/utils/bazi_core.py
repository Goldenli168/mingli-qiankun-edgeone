...
# 三刑 (梁湘润：寅巳申无恩之刑·丑戌未恃势之刑·子卯无礼之刑·辰午酉亥自刑)
_SANXING = {
    "寅": ["巳", "申"], "巳": ["寅", "申"], "申": ["寅", "巳"],
    "丑": ["戌", "未"], "戌": ["丑", "未"], "未": ["丑", "戌"],
    "子": ["卯"], "卯": ["子"],
...
def _liunian_brief(y, g, z, ssg, ssz, chong, he, fp, score, dy_ssg, bz, xy):
    ...
    tianyi = {"甲":["丑","未"],"乙":["子","申"],"丙":["亥","酉"],"丁":["亥","酉"],"戊":["丑","未"],"己":["子","申"],"庚":["丑","未"],"辛":["午","寅"],"壬":["卯","巳"],"癸":["卯","巳"]}
    wenchang = {"甲":"巳","乙":"午","丙":"申","丁":"酉","戊":"申","己":"酉","庚":"亥","辛":"子","壬":"寅","癸":"卯"}
    if yima_map.get(nz2) == z: ss_notes.append("今年适合走出去——出差、旅行、换城市都行，窝在家里反而错过机会")
    if taohua_map.get(nz2) == z: ss_notes.append("桃花运在线，单身的多出去社交，已婚的把注意力放在伴侣身上")
    if z in tianyi.get(dg, []): ss_notes.append("有贵人在暗处盯着你——遇到困难别硬扛，开口求助反而有人拉你一把")
    if z == wenchang.get(dg, ""): ss_notes.append("考试运不错，适合报班学习或者写点东西")
    if ss_notes:
