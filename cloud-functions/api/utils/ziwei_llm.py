...
def parse_verify(text: str, result, birth_year: int):
    ...
    palace_re = _re.compile(r"([^\s，。、·:：→|｜/()（）「」]{1,4})宫")
    ...
    for line in text.split("\n"):
        ...
        # 三判定:token以12宫名结尾(落福德宫/冲父母宫) 或 token+宫本身即宫名(→命宫)
        ...
        if ok and birth_year:
            ...
            if _years:
                ...
                if _ctype in ("marriage", "children") and _age_at < 20:
                    ...
                    ok = False
        # ④b "或"字零容忍的代码兜底(P81v5):prompt已禁但LLM实测仍约30%违规→直接剔除
        if ok and any(w in claim for w in ("或", "可能", "大概")):
            ok = False
            ...
            if _row0:
                ...
                if ok and _re.search(r"生子|添丁|怀孕|生育|得子|得女|二胎", claim):
                    if not (_row0["ss"] in _child_ss or "子女" in _row0["ln_ming"]
                        ...
                        ok = False
                # ⑤d 置业类:该年必须有田宅信号(命入田宅/四化入田宅/限入田宅大限)
                if ok and _re.search(r"买房|购房|置业|卖房|装修|搬家|迁居|乔迁", claim):
                    if not ("田宅" in _row0["ln_ming"] or "→田宅" in _row0["sihua_text"]
                            or "田宅" in _row0["dx"]):
                        ok = False
...
【应期规则-信号分权重,严禁只看四化选年】
- 婚恋(恋爱/结婚): 权重①配偶星年(男命正财/偏财年、女命正官/七杀年) ②流年化禄/化权/化科入夫妻宫 ③流年命入夫妻宫 ④红鸾天喜动(仅辅助加分)。⚠️①②③全无的年份,即使红鸾/天喜动也严禁断结婚领证——红鸾天喜是"喜庆星",婚恋/添丁/庆典都可能应,不是结婚专属铁证(实测教训:2014仅有天喜动被误断结婚,实为添丁;真正的结婚年2012=正官年+化禄入夫妻宫)
- 子女(怀孕/生育): 权重①子女星年(男命正官/七杀年、女命食神/伤官年) ②流年命入子女宫或四化入子女宫 ③红鸾天喜动(添丁亦主喜庆)。生育断语年份必须晚于结婚断语年份(不主动断未婚先孕)
- 学业考试: 正印/偏印年; 化科(文昌/文曲化科分量最重)
- 置业迁居(买房/搬家/装修): 只允许在"流年命入田宅宫、四化(禄权科忌任一)入田宅宫、所在大限为田宅宫"三类信号至少占一类的年份下断——三类全无的年份严禁断买房置业(实测教训:曾断无任何田宅信号的年份买房)
- 事业变动: 正官/七杀/正偏财年; 化禄/化权入官禄宫; 流年命入官禄宫
- 健康伤病: 化忌入疾厄宫; 流年命入疾厄宫
...
⚠️ 天干相同的年份四化完全相同(如2012与2022同为壬年),必须用流年命宫/大限/十神区分,只凭四化选年必错
...
直接输出7行,不要开场白。"""
    ...
    try:
        ...
        # P81: verify的prompt v6(信号权重排序:红鸾天喜降权为辅助;置业/亏损/分娩方式三类纠偏;
        # v38加候选年比选+或/红鸾/十神代码校验)→独立递增,不影响其他gen_type缓存
        _ck_ver = "v39" if gen_type == "verify" else "v33"
