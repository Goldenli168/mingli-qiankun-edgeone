            ...
            if _row0:
                ...
                if ok and _re.search(r"生子|添丁|怀孕|生育|得子|得女|二胎", claim):
                    if not (_row0["ss"] in _child_ss or "子女" in _row0["ln_ming"]
                        ...
                        ok = False
                # ⑤d 置业迁居类(P81v8收紧):合格信号仅"禄/权/忌→田宅"或"限入田宅大限"——
                # 命入田宅只是位置不是动作、化科=文书名声分量太轻(实测:2015乙未命入田宅+科紫微→田宅被误断买房,
                # 命主并未置业;真实置业年2012权→田宅/2019忌→田宅/2023禄→田宅+限田宅全是重量级信号)
                if ok and _re.search(r"买房|购房|置业|卖房|装修|搬家|迁居|乔迁", claim):
                    if not (_re.search(r"[禄权忌][^ ]*→田宅", _row0["sihua_text"])
                            or "田宅" in _row0["dx"]):
                        ok = False
...
- 置业迁居(买房/卖房/搬家/迁居/装修): 只允许在"化禄/化权/化忌入田宅宫、所在大限为田宅宫"两类信号至少占一类的年份下断。⚠️单化科入田宅分量太轻(科=文书名声,不是重资产动作),严禁据"科入田宅"断买房搬家——即使同年命入田宅也不算(实测教训:2015乙未命入田宅+紫微化科入田宅被误断买房,命主该年并未置业;真实置业年全是禄/权/忌入田宅的重量级信号:2012权紫微→田宅/2019忌文曲→田宅/2023禄破军→田宅+限田宅)。⚠️迁移宫化忌=在外奔波受挫,严禁据此断搬家(实测教训:2005乙酉太阴化忌入迁移被误断搬家,命主该年并未搬家)
