            ...
            if _row0:
                ...
                if _re.search(r"剖腹产|顺产", claim):
                    ...
                    if _ym2 and _gzm2:
                        ...
                        ok = False
                # ⑤c 婚恋类:该年必须占①配偶星/②四化入夫妻/③命入夫妻至少其一(P81v7:红鸾天喜不再算合格信号——
...
直接输出7行,不要开场白。"""
    ...
    try:
        ...
        # P81: verify的prompt v7(⑤c红鸾天喜降为纯辅助:恋爱同样不得单凭红鸾下断;同天干年必须选大限/命宫加持年;
        # v6=信号权重排序+置业/亏损/分娩方式纠偏)→独立递增,不影响其他gen_type缓存
        _ck_ver = "v40" if gen_type == "verify" else "v33"
